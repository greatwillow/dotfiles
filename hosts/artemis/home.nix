...
let
  ...
  shellAliases =
    (import "${homeManagerModulesPath}/_common/shell_aliases.nix" {
      inherit pkgs homeManagerModulesPath;
    }).shellAliases;
  sessionVariables =
    (import "${homeManagerModulesPath}/_common/session_variables.nix" {
      inherit
        pkgs
        rootConfigPath
        isMacOS
...
in
{
  ...
  programs = {
    ...
    bash =
      (import "${homeManagerModulesPath}/bash/bash.nix" {
        inherit pkgs shellAliases homeManagerModulesPath;
      }).bash;
    zsh =
      (import "${homeManagerModulesPath}/zsh/zsh.nix" {
        inherit pkgs shellAliases homeManagerModulesPath;
      }).zsh;
    nushell = {
      enable = true;
